...
module WinthropClient
  class TeamScheduleDetailSeason < ApiModelBase
    attr_accessor :wins
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::TeamScheduleDetailSeason` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::TeamScheduleDetailSeason`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [wins, losses, record_str, rpi, net_rank, ap_rank, avg_rpi, avg_net_rank, avg_ap_rank].hash
    end
