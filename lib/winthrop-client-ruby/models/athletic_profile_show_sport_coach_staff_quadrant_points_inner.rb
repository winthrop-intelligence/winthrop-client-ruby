...
module WinthropClient
  class AthleticProfileShowSportCoachStaffQuadrantPointsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::AthleticProfileShowSportCoachStaffQuadrantPointsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::AthleticProfileShowSportCoachStaffQuadrantPointsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [school_id, name, short_name, colors, is_subject, coach_name, coach_last_name, comp_cents, comp_basis, comp_fiscal_year, net_rank, rpi, conference_wins, record].hash
    end
