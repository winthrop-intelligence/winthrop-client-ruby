...
module WinthropClient
  ...
  class Filters < ApiModelBase
    ...
    def self.openapi_types
      {
        :'priority_ids' => :'Array<Integer>',
        :'page' => :'Integer',
        :'per_page' => :'Integer',
        :'q' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::Filters` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::Filters`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'q')
        if (value = attributes[:'q']).is_a?(Hash)
          self.q = value
        end
      end
    end
    ...
    def hash
      [priority_ids, page, per_page, q].hash
    end
