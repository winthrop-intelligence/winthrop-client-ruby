...
module WinthropClient
  ...
  class DepartmentAdministratorsAdProfile < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::DepartmentAdministratorsAdProfile` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::DepartmentAdministratorsAdProfile`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'in_seat_since')
        self.in_seat_since = attributes[:'in_seat_since']
      else
        self.in_seat_since = nil
      end
    ...
    def hash
      [coach_id, friendly_id, name, title, in_seat_since, since_year, years_at_school].hash
    end
