...
module WinthropClient
  class FoiaFollowUpRequestedItem < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::FoiaFollowUpRequestedItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::FoiaFollowUpRequestedItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ri_note_id')
        self.ri_note_id = attributes[:'ri_note_id']
      end
    ...
    def hash
      [requested_item_id, requestable_type, requestable_id, requestable_data, type_display, title, status, created_at, updated_at, status_changed_at, previous_status, received_at, coach_id, coach_name, ri_note_id, note, note_updated_at].hash
    end
