...
module WinthropClient
  class GameContractSeriesEntry < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::GameContractSeriesEntry` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::GameContractSeriesEntry`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, sport_name, home_school_id, home_school_name, away_school_id, away_school_name, comp_cents, comp_tbd, variable, cancel_fee_cents, cancelled, season_year, game_date, game_date_tbd].hash
    end
