...
module WinthropClient
  class RecordPositionEntry < ApiModelBase
    ...
    attr_accessor :us_news

    attr_accessor :directors_cup
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WinthropClient::RecordPositionEntry` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WinthropClient::RecordPositionEntry`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'us_news')
        self.us_news = attributes[:'us_news']
      end

      if attributes.key?(:'directors_cup')
        self.directors_cup = attributes[:'directors_cup']
      end
    end
    ...
    def hash
      [sport, year_str, school_name, school_id, position_sport, record, rpi, net_rank, ap_rank, apr_asr, coach_apr, departing, us_news, directors_cup].hash
    end
