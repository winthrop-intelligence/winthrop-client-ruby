...
module WinthropClient
  class ScraperApi
    ...
    # @param job_name [String] The Kubernetes Job name to delete
    # @param [Hash] opts the optional parameters
    # @return [nil]
    def delete_running_job(job_name, opts = {})
      delete_running_job_with_http_info(job_name, opts)
      nil
    end
    ...
    def delete_running_job_with_http_info(job_name, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ScraperApi.delete_running_job ...'
      end
      # verify the required parameter 'job_name' is set
      if @api_client.config.client_side_validation && job_name.nil?
        fail ArgumentError, "Missing the required parameter 'job_name' when calling ScraperApi.delete_running_job"
      end
      # resource path
      local_var_path = '/ondemand-scrapers/jobs/{job_name}'.sub('{' + 'job_name' + '}', CGI.escape(job_name.to_s))
      ...
      new_options = opts.merge(
        :operation => :"ScraperApi.delete_running_job",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ScraperApi#delete_running_job\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    # List still-running Kubernetes Jobs started by the run endpoint
    # @param [Hash] opts the optional parameters
    # @return [Array<(Array<RunningJob>, Integer, Hash)>] Array<RunningJob> data, response status code and response headers
    def list_jobs_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ScraperApi.list_jobs ...'
      end
      # resource path
      local_var_path = '/ondemand-scrapers/jobs'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'Array<RunningJob>'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKey', 'Oauth2']

      new_options = opts.merge(
        :operation => :"ScraperApi.list_jobs",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ScraperApi#list_jobs\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
