...
module WinthropClient
  class ScraperApi
    ...
    # Run a scraper
    # @param scraper [String] The name of the scraper to run
    # @param [Hash] opts the optional parameters
    # @return [nil]
    def run_scraper(scraper, opts = {})
      run_scraper_with_http_info(scraper, opts)
      nil
    end
    ...
    def run_scraper_with_http_info(scraper, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ScraperApi.run_scraper ...'
      end
      # verify the required parameter 'scraper' is set
      if @api_client.config.client_side_validation && scraper.nil?
        fail ArgumentError, "Missing the required parameter 'scraper' when calling ScraperApi.run_scraper"
      end
      # resource path
      local_var_path = '/ondemand-scrapers/run/{scraper}'.sub('{' + 'scraper' + '}', CGI.escape(scraper.to_s))
