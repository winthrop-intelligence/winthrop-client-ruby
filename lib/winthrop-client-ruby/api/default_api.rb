...
module WinthropClient
  class DefaultApi
    ...
    # @param requested_item_id [Integer] ID of requested item whose note should be created or appended
    ...
    # @param windows [Array<String>] 1-10 inclusive date windows as YYYY-MM-DD..YYYY-MM-DD ranges
    # @param [Hash] opts the optional parameters
    # @option opts [Object] :q Ransack query
    # @return [GamePostGapCountCollection]
    def get_game_post_search_gap_counts(windows, opts = {})
      data, _status_code, _headers = get_game_post_search_gap_counts_with_http_info(windows, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Object] :q Ransack query
    # @return [Array<(GamePostGapCountCollection, Integer, Hash)>] GamePostGapCountCollection data, response status code and response headers
    def get_game_post_search_gap_counts_with_http_info(windows, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DefaultApi.get_game_post_search_gap_counts ...'
      end
      # verify the required parameter 'windows' is set
      if @api_client.config.client_side_validation && windows.nil?
        fail ArgumentError, "Missing the required parameter 'windows' when calling DefaultApi.get_game_post_search_gap_counts"
      end
      if @api_client.config.client_side_validation && windows.length > 10
        fail ArgumentError, 'invalid value for "windows" when calling DefaultApi.get_game_post_search_gap_counts, number of items must be less than or equal to 10.'
      end

      if @api_client.config.client_side_validation && windows.length < 1
        fail ArgumentError, 'invalid value for "windows" when calling DefaultApi.get_game_post_search_gap_counts, number of items must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/api/v1/game_post_searches/gap_counts'

      # query parameters
      query_params = opts[:query_params] || {}
      query_params[:'windows'] = @api_client.build_collection_param(windows, :multi)
      query_params[:'q'] = opts[:'q'] if !opts[:'q'].nil?
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GamePostGapCountCollection'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['ApiKey', 'Oauth2']

      new_options = opts.merge(
        :operation => :"DefaultApi.get_game_post_search_gap_counts",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DefaultApi#get_game_post_search_gap_counts\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
