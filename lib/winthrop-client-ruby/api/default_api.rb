...
module WinthropClient
  class DefaultApi
    ...
    # @option opts [String] :net_ranking_tier Restrict to a NET ranking band (latest non-null NET rank for the requested sport). Schools without a NET rank are excluded from every named tier; omit the param to leave results unfiltered.
    ...
    def get_schedule_grid_available_schools_with_http_info(sport_name, target_date, opts = {})
      ...
      allowable_values = ["top_50", "51_100", "101_200", "201_plus"]
      if @api_client.config.client_side_validation && opts[:'net_ranking_tier'] && !allowable_values.include?(opts[:'net_ranking_tier'])
        fail ArgumentError, "invalid value for \"net_ranking_tier\", must be one of #{allowable_values}"
      end
