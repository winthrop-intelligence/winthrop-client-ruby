...
begin
  ...
  result = api_instance.get_desk_reports
  p result
rescue WinthropClient::ApiError => e
...
> <Array(<DeskReportsResponse>, Integer, Hash)> get_desk_reports_with_http_info

```ruby
begin
  
  data, status_code, headers = api_instance.get_desk_reports_with_http_info
  p status_code # => 2xx
  p headers # => { ... }
